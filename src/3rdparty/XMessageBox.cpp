...
protected:
	...
	COLORREF	m_crBackground;		// message and button background color
	COLORREF	m_crBackgroundAlt;	// non-message area background color
	HBRUSH		m_hBackgroundBrush;	// background brush for message and button
	HBRUSH		m_hBackgroundBrushAlt;	// background brush for non-message area
...
CXDialogTemplate::CXDialogTemplate(HWND hWnd,
	...
	m_crBackgroundAlt(pXMB->crBackgroundAlt),
	...
	m_bNarrow(pXMB->dwOptions & XMSGBOXPARAMS::Narrow)				//+++1.8
{
	...
	if (m_crBackgroundAlt != CLR_INVALID)
		m_hBackgroundBrushAlt = ::CreateSolidBrush(m_crBackgroundAlt);
...
CXDialogTemplate::~CXDialogTemplate()
{
	...
	if (m_hBackgroundBrushAlt)
		::DeleteObject(m_hBackgroundBrushAlt);
	m_hBackgroundBrushAlt = 0;
												 ...
												 LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_DRAWITEM:
		{
			...
			dc.FillSolidRect(&rect, Me->m_crBackground);
			dc.Draw3dRect(&rect, Me->m_crText, Me->m_crText);

			dc.SetBkColor(Me->m_crBackground);
			dc.SetTextColor(Me->m_crText);
		...
		case WM_ERASEBKGND:			//+++1.8
		{
			if (Me->m_hBackgroundBrush)
			{
				...
				if (Me->m_hBackgroundBrushAlt)
				{
					::FillRect(hdc, &rect, Me->m_hBackgroundBrushAlt);
				}
				else
				{
...
BOOL CXDialogTemplate::OnKeyDown(HWND hWnd, WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if (bCtrlIsDown && (ch == _T('C') || ch == VK_INSERT))
	{
		rc = TRUE;
						   ...
						   LPCTSTR lpszCaption)
{
	...
	switch (ctrltype)
	{
		...
		case BUTTON:
			...
			m_dlgItemTemplate.style = WS_VISIBLE | WS_CHILD | WS_TABSTOP | BS_OWNERDRAW;
