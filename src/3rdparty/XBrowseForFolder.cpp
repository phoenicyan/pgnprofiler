...
{
	ATLTRACE2(atlTraceDBProvider, 0, _T("in void SizeBrowseDialog\n"));
	...
	if (!hwndTree)
	{
		...
		ATLTRACE2(atlTraceDBProvider, 0, _T("SHBrowseForFolder ShellNameSpace Control: hwndTree=%X\n"), hwndTree);
	}
	...
	if (hwndTree)
	{
		...
		ATLTRACE2(atlTraceDBProvider, 0, _T("hwndEdit=%x\n"), hwndEdit);
		CRect rectEdit;
	...
	{
		ATLTRACE2(atlTraceDBProvider, 0, _T("ERROR - tree control not found.\n"));
		//_ASSERTE(hwndTree);
	}
}
														...
														// BROWSEINFO structure
{
	switch (uMsg)
	{
		...
		{
			ATLTRACE2(atlTraceDBProvider, 0, _T("hWnd=%X\n"), hWnd);
		...
		case BFFM_SELCHANGED:		// sent when the selection has changed
		{
			...
			if (bRet)
			{
				...
				else
				{
					...
					ATLTRACE2(atlTraceDBProvider, 0, _T("dwAttributes=0x%08X\n"), sfi.dwAttributes);

					// fail if pidl is a link
					if (sfi.dwAttributes & SFGAO_LINK)
					{
						ATLTRACE2(atlTraceDBProvider, 0, _T("SFGAO_LINK\n"));
						bRet = FALSE;
					}
				}
			}
			...
			ATLTRACE2(atlTraceDBProvider, 0, _T("szDir=%s\n"), szDir);
		}
		break;
	}
					  ...
					  BOOL bEditBox /*= FALSE*/)
{
	...
	if (lpszInitialFolder)
	{
		...
		if (HIWORD(lpszInitialFolder) == 0)
		{
			...
			ATLTRACE2(atlTraceDBProvider, 0, _T("csidl:  nFolder=0x%X\n"), nFolder);
		...
		ATLTRACE2(atlTraceDBProvider, 0, _T("szInitialPath=<%s>\n"), szInitialPath);
	}
	...
	if (pidlFolder)
	{
		...
		{
			ATLTRACE2(atlTraceDBProvider, 0, _T("SHGetPathFromIDList failed\n"));
		}
	}
